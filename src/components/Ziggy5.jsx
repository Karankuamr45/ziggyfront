...
const Ziggy5 = () => {
  ...
  const [newRestaurantDescription, setNewRestaurantDescription] = useState(''); // Added
  const [newRestaurantRatings, setNewRestaurantRatings] = useState(''); // Added
  ...
  const handleRestaurantSubmit = async (e) => {
    ...
    formData.append('description', newRestaurantDescription); // Add description field
    formData.append('ratings', newRestaurantRatings);
    ...
    try {
      ...
      setNewRestaurantDescription(''); // Reset description field
      setNewRestaurantRatings(''); // Reset ratings field
  ...
  const handleItemSubmit = async (e, restaurantId) => {
    ...
    formData.append('description', newItemInputs[restaurantId]?.description || ''); // Add this line for description
    ...
    try {
      ...
      setNewItemInputs((prevInputs) => ({
        ...prevInputs,
        [restaurantId]: { name: '', price: '', image: null, description: '' },
      }));
    } catch (error) {
    ...
    <div className="container mx-auto p-4">
      ...
      <form onSubmit={handleRestaurantSubmit} className="mb-8">
        ...
        <label className="block mb-2">
          ...
          <input
            type="text"
            value={newRestaurantRatings}
            onChange={(e) => setNewRestaurantRatings(e.target.value)}
            className="border p-2 w-full sm:w-1/2"
          />
        </label>
        <label className="block mb-2">
          ...
          <div key={restaurant._id} className="border p-4">
            ...
            <form onSubmit={(e) => handleItemSubmit(e, restaurant._id)} className="mb-4">
              ...
              <label className="block mb-2">
                ...
                <textarea
                  type="text"
                  value={newItemInputs[restaurant._id]?.description || ''}
                  onChange={(e) => handleItemInputChange(restaurant._id, 'description', e.target.value)}
                  className="border p-2 w-full"
                />
              </label>
